...
public struct OnBoardingView: View {
    ...
    var viewModel: StateObject<OnBoardingViewModel>
    @State private var buttonClicked: Bool = false
    
    public init(viewModel: StateObject<OnBoardingViewModel>) {
    ...
    public var body: some View {
        ZStack {
            ...
            VStack {
                ...
                Group {
                    switch viewModel.wrappedValue.state {
                    case .authorized(_):
                        Text("Succes")
                    case .notDetermined:
                        VStack {
                            Button {
                                viewModel.wrappedValue.checkIfLocationServiceIsEnabled()
                                buttonClicked = true
                            } label: {
                                HStack {
        ...
        .onAppear() {
            if buttonClicked { viewModel.wrappedValue.checkIfLocationServiceIsEnabled() }
        }
    }
}
